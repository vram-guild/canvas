...
const float[8] FRX_BITWISE_DIVISORS = float[8](0.5, 0.25, 0.125, 0.0625, 0.03125, 0.015625, 0.0078125, 0.00390625);
...
float frx_bitValue(float byteValue, int bitIndex) {
    return floor(fract(byteValue * FRX_BITWISE_DIVISORS[bitIndex]) * 2.0);
}
