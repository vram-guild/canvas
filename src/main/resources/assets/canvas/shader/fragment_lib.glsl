...
vec4 combinedLight(float offsetU, float offsetV) {
    ...
    vec4 sky = texture2D(u_utility, v_hd_skylight + offset);
//    float s = floor(sky.r * 256.0);
//    float c = s == 247 ? 8.0/256.0 : sky.r;
//    vec2 lightCoord = vec2(8.0/255.0, 8.0/255.0); //vec2(block.r, sky.r);
...
//    return texture2D(u_lightmap, v_hd_skylight);
}
#endif
