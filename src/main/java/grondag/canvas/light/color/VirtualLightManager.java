...
public class VirtualLightManager implements LightView {
    private BlockAndTintGetter baseView = null;

    private final Long2ObjectOpenHashMap<ShortArrayList> positions = new Long2ObjectOpenHashMap<>();
    private final LongArrayFIFOQueue queue = new LongArrayFIFOQueue();
    private final ObjectOpenHashSet<LightRegion> checkQueue = new ObjectOpenHashSet<>();
    ...
    void startFrame(BlockAndTintGetter blockView) {
        baseView = blockView;
        update();
    }
    ...
    @Override
    public BlockAndTintGetter baseView() {
        return baseView;
    }

    @Override
    public short getRegistered(BlockPos pos) {
        var registered = baseView == null ? 0 : LightRegistry.get(baseView.getBlockState(pos));
    ...
    public short getRegistered(BlockPos pos, @Nullable BlockState state) {
        ...
        var registered = state != null ? LightRegistry.get(state) : (baseView == null ? 0 : LightRegistry.get(baseView.getBlockState(pos)));
        return combineWithBlockLight(registered, getVirtualLight(pos));
    }
    ...
    public void placeVirtualLight(BlockPos blockPos, short light) {
        ...
        final var list = positions.computeIfAbsent(pos, l -> new ShortArrayList());
    ...
    public void removeVirtualLight(BlockPos blockPos, short light) {
        final long pos = blockPos.asLong();
        final var list = positions.get(pos);
    ...
    private short getVirtualLight(BlockPos blockPos) {
        ShortArrayList lights = positions.get(blockPos.asLong());
    ...
    private void update() {
        while (!queue.isEmpty()) {
            ...
            var region = LightDataManager.INSTANCE.get(pos);

            if (region != null) {
                var blockPos = BlockPos.of(pos);
                region.checkBlock(blockPos, null);
                checkQueue.add(region);
            } else if (positions.containsKey(pos)) {
    ...
    private static short encodeVirtualLight(short light) {
