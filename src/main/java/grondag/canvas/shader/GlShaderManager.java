...
import grondag.canvas.wip.shader.WipGlShader;
...
public enum GlShaderManager {
	...
	public Shader getOrCreateVertexShader(Identifier shaderSource, WipProgramType programType) {
		...
		synchronized (vertexShaders) {
			...
			if (result == null) {
				result = new WipGlShader(shaderSource, GL21.GL_VERTEX_SHADER, programType);
	...
	public Shader getOrCreateFragmentShader(Identifier shaderSourceId, WipProgramType programType) {
		...
		synchronized (fragmentShaders) {
			...
			if (result == null) {
				result = new WipGlShader(shaderSourceId, GL21.GL_FRAGMENT_SHADER, programType);
