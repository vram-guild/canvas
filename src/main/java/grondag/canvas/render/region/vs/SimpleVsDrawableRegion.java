...
public class SimpleVsDrawableRegion extends AbstractDrawableRegion<SimpleVsDrawableState> {
	private SimpleVsDrawableRegion(SimpleVsDrawableState delegate, long packedOriginBlockPos) {
	...
	static DrawableRegion pack(VertexCollectorList collectorList, boolean translucent, int byteCount, long packedOriginBlockPos) {
		...
		SimpleVsDrawableStorage storage = new SimpleVsDrawableStorage(transferBuffer, byteCount);

		final SimpleVsDrawableState drawState = new SimpleVsDrawableState(collector.renderState, collector.quadCount() * 4, storage);
		return new SimpleVsDrawableRegion(drawState, packedOriginBlockPos);
	}
