...
public class NaiveVsRegionRenderConfig2 extends RegionRenderConfig {
	public static final NaiveVsRegionRenderConfig2 INSTANCE = new NaiveVsRegionRenderConfig2();

	private NaiveVsRegionRenderConfig2() {
		super(
			VsFormat.VS_MATERIAL,
			VsFormat.VS_MATERIAL.quadStrideInts,
			true,
			VsFormat.VS_TRANSCODER,
			NaiveVsDrawList2::build // <- only difference
		);
	}
	...
	public UploadableRegion createUploadableRegion(VertexCollectorList vertexCollectorList, boolean sorted, int bytes, long packedOriginBlockPos) {
		return new NaiveVsUploadableRegion(vertexCollectorList, sorted, bytes, packedOriginBlockPos);
	}
