...
public abstract class GeometryHelper {
    ...
    /**
     * Analyzes the quad and returns a value with some combination of
     * {@link #AXIS_ALIGNED_FLAG}, {@link #LIGHT_FACE_FLAG} and {@link #CUBIC_FLAG}.
     * Intended use is to optimize lighting when the geometry is regular. Expects
     * convex quads with all points co-planar.
     */
    ...
    /**
     * Returns true if quad is parallel to the given face. Does not validate quad
    ...
    /**
     * True if quad - already known to be parallel to a face - is actually coplanar
     * with it.
     * <p>
     * 
     * Test will be unreliable if not already parallel, use
     * {@link #isQuadParallel(Direction, QuadView)} for that purpose. Expects convex
     * quads with all points co-planar.
     * <p>
     */
    ...
    /**
     * Returns true if quad is truly a quad (not a triangle) and fills a full block
     * cross-section. If known to be true, allows use of a simpler/faster AO
     * lighting algorithm.
     * <p>
     * 
     * Does not check if quad is actually coplanar with the light face, nor does it
     * check that all quad vertices are coplanar with each other.
     * <p>
     * 
    ...
    /**
     * Identifies the face to which the quad is most closely aligned. This mimics
     * the value that {@link BakedQuad#getFace()} returns, and is used in the
     * vanilla renderer for all diffuse lighting.
     * <p>
     * 
     * Derived from the quad face normal and expects convex quads with all points
     * co-planar.
     */
    ...
    /**
     * Identifies the largest (max absolute magnitude) component (X, Y, Z) in the
     * given vector.
     */
