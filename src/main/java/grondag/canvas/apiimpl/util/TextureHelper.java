 ...
 * via {@link #bakeSprite(MutableQuadView, int, Sprite, int)}. Also used by the
 * API itself to implement automatic block-breaking models for enhanced models.
 */
public class TextureHelper {
	 ...
	 * interpolation, etc. Textures must not be already baked.
	 */
	public static void bakeSprite(MutableQuadView quad, int spriteIndex, Sprite sprite, int bakeFlags) {
		if (quad.nominalFace() != null && (MutableQuadView.BAKE_LOCK_UV & bakeFlags) != 0) {
			...
			applyModifier(quad, spriteIndex,
					(q, i, t) -> q.sprite(i, t, q.spriteU(i, t) * NORMALIZER, q.spriteV(i, t) * NORMALIZER));
		}
	 ...
	 * call, so we'd have to denormalize before we called, only to have the sprite
	 * renormalize immediately.
	 */
	private static void interpolate(MutableQuadView q, int spriteIndex, Sprite sprite) {
		...
		for (int i = 0; i < 4; i++) {
			q.sprite(i, spriteIndex, uMin + q.spriteU(i, spriteIndex) * uSpan,
					vMin + q.spriteV(i, spriteIndex) * vSpan);
		}
	}
