...
public abstract class NormalHelper {
    ...
    /**
     * Stores a normal plus an extra value as a quartet of signed bytes. This is the
     * same normal format that vanilla item rendering expects. The extra value is
     * for use by shaders.
     */
    ...
    /**
     * Version of {@link #packNormal(float, float, float, float)} that accepts a
     * vector type.
     */
    ...
    /**
     * Computes the face normal of the given quad and saves it in the provided
     * non-null vector. If {@link QuadView#nominalFace()} is set will optimize by
     * confirming quad is parallel to that face and, if so, use the standard normal
     * for that face direction.
     * <p>
     * 
     * Will work with triangles also. Assumes counter-clockwise winding order, which
     * is the norm. Expects convex quads with all points co-planar.
     */
