...
public class ItemRenderContext extends AbstractRenderContext implements RenderContext {
	...
	private final TessellatorExt tessellatorExt = (TessellatorExt)tessellator;
	private final CanvasBufferBuilder canvasBuilder = (CanvasBufferBuilder) tessellator.getBufferBuilder();
	private int color;
	...
	private final Consumer<Mesh> meshConsumer = (mesh) -> {
		...
		while (index < limit) {
			...
			System.arraycopy(data, index, quadData, 0, stride);
			editorQuad.load();
			index += stride;
			renderQuad();
		}
	...
	private final Consumer<BakedModel> fallbackConsumer = model -> {
		...
		for (int i = 0; i < 7; i++) {
			...
			final List<BakedQuad> quads = model.getQuads(NULL_BLOCK_STATE, face, random);
