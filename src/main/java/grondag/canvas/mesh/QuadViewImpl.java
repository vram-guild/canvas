...
import static grondag.canvas.mesh.EncodingFormat.HEADER_BITS;
import static grondag.canvas.mesh.EncodingFormat.HEADER_COLOR_INDEX;
import static grondag.canvas.mesh.EncodingFormat.HEADER_MATERIAL;
import static grondag.canvas.mesh.EncodingFormat.HEADER_TAG;
import static grondag.canvas.mesh.EncodingFormat.NORMALS_OFFSET;
import static grondag.canvas.mesh.EncodingFormat.SECOND_TEXTURE_OFFSET;
import static grondag.canvas.mesh.EncodingFormat.TEXTURE_OFFSET_MINUS;
import static grondag.canvas.mesh.EncodingFormat.TEXTURE_STRIDE;
import static grondag.canvas.mesh.EncodingFormat.THIRD_TEXTURE_OFFSET;
...
public class QuadViewImpl implements QuadView {
    ...
    public float normExtra(int vertexIndex) {
        return hasNormal(vertexIndex)
                ? NormalHelper.getPackedNormalComponent(data[baseIndex + VERTEX_START_OFFSET + VANILLA_STRIDE + vertexIndex], 3)
                : Float.NaN;
    }
