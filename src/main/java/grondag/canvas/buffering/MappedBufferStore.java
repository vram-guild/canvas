...
public class MappedBufferStore {
    ...
    private static final ArrayBlockingQueue<MappedBuffer> emptyMapped = new ArrayBlockingQueue<MappedBuffer>(
            TARGET_BUFFERS);
     ...
     */
    private static final ArrayBlockingQueue<MappedBuffer> releaseRebufferQueue = new ArrayBlockingQueue<MappedBuffer>(
            64);
    ...
    /**
     * Called at start of each render tick from client thread to maintain a pool of
     * mapped buffers ready for off-thread loading.
     */
...
//    }
    ...
    /**
     * Called by mapped buffers when they are released off thread. Prevents GL calls
